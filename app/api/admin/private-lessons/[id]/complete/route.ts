...
import { completePrivateLesson } from "@/lib/private-lessons";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const lesson = await completePrivateLesson(id, await request.json(), user!._id);
    return successResponse({ lesson });
  } catch (err) {
    console.error("Private lesson complete:", err);
