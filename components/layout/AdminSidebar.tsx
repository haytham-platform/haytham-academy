...
import { Archive, Bell, BookOpen, Bot, Bus, CalendarRange, ClipboardList, FileBarChart, GraduationCap, LayoutDashboard, LogOut, MessageSquare, Newspaper, Settings, Users, Wallet } from "lucide-react";
import { cn } from "@/lib/utils";
import { hasPermission, type Permission } from "@/lib/permissions";
import AcademyLogo from "@/components/ui/AcademyLogo";
import type { UserRole } from "@/types";

const adminLinks: { href: string; label: string; icon: React.ElementType; permission: Permission }[] = [
  ...
  { href: "/admin/communications", label: "مركز التواصل", icon: MessageSquare, permission: "communications.view" },
  { href: "/admin/notifications", label: "الإشعارات", icon: Bell, permission: "notifications.view" },
  ...
  { href: "/admin/ai", label: "منصة الذكاء الاصطناعي", icon: Bot, permission: "ai.admin" },
...
export default function AdminSidebar() {
  ...
  const visibleLinks = adminLinks.filter((link) => role && hasPermission(role, link.permission));
    ...
    <aside className="flex w-full shrink-0 flex-col border-b border-border bg-primary text-white md:min-h-screen md:w-64 md:border-b-0 md:border-l">
      ...
      <nav className="flex flex-1 flex-col gap-1 p-3" aria-label="قائمة الإدارة">
        {visibleLinks.map(({ href, label, icon: Icon }) => (
          <Link key={href} href={href} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition", pathname === href || pathname.startsWith(`${href}/`) ? "bg-secondary text-white" : "text-pink-100 hover:bg-white/10 hover:text-white")}>
      ...
      <div className="border-t border-white/10 p-3">
        <button type="button" onClick={handleLogout} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-red-300 transition hover:bg-red-500/10">
