...
import { Baby, BarChart3, Bell, Bus, CalendarCheck, FileText, GraduationCap, LayoutDashboard, LogOut, ReceiptText, Users } from "lucide-react";
      ...
      <div className="border-b border-border p-5">
        <Link href="/" className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted">بوابة الأولياء</p>
          </div>
        </Link>
      </div>
      <nav className="flex flex-1 flex-col gap-1 p-3" aria-label="قائمة ولي الأمر">
        {parentLinks.map(({ href, label, icon: Icon }) => (
          <Link
            key={href}
            href={href}
            className={cn(
