...
import { Bell, BookOpen, Bot, CalendarCheck, FileText, GraduationCap, LayoutDashboard, LogOut, ReceiptText, User } from "lucide-react";
...
const studentLinks = [
  ...
  { href: "/student/ai", label: "المساعد الذكي", icon: Bot },
];
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col border-b border-border bg-white md:min-h-screen md:w-64 md:border-b-0 md:border-l">
      <div className="border-b border-border p-5">
        <Link href="/" className="flex items-center gap-2"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary text-white"><GraduationCap className="h-4 w-4" /></div><div><p className="text-xs font-bold text-primary">{ACADEMY.name}</p><p className="text-[10px] text-muted">بوابة الطالب</p></div></Link>
      </div>
      <nav className="flex flex-1 flex-col gap-1 p-3" aria-label="قائمة الطالب">
        {studentLinks.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition", pathname === href || pathname.startsWith(`${href}/`) ? "bg-pink-50 text-primary" : "text-foreground hover:bg-gray-50 hover:text-primary")}><Icon className="h-4 w-4" />{label}</Link>)}
      </nav>
